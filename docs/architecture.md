...
Apple Peripherals for Windows is a native Windows desktop application written in C# on .NET 8. The app is split into small layers so HID access, gesture interpretation, settings, and Windows input injection can be tested and changed independently.
...
- `Hid` identifies Apple Magic Trackpad devices, sends feature reports, and parses Apple 9-byte multitouch records.
- `Runtime` hosts the hidden Raw Input window, reacts to Bluetooth reconnect/device-change messages, refreshes multitouch mode, and reloads settings while the bridge runs.
...
The settings file is shared by the app and bridge. The running bridge watches the file timestamp and reloads gesture settings, sensitivity, direction, click mappings, and keybinds after the settings app saves.
